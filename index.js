...
console.log(chalk.bold.bgRedBright("WELCOME TO - A MULTILEVEL QUIZ ON SUITS\n"));
...
console.log(chalk.bold.blue("\nWelcome " +userName+ "! Let's See How much do you know about USA’s Suits ? \n"))
...
Enter Your Choice: `),
  answer:"b",
  correctAnswer: "He needs money to pay for his grandmother’s nursing home care."
},
{
...
for(var i=0;i<levelOneQuestions.length;i++){
  play(levelOneQuestions[i].question,levelOneQuestions[i].answer,levelOneQuestions[i].correctAnswer);
}
totalScore(score);
...
if(levelOneScore >= 23){
  ...
  for(var i=0;i<levelTwoQuestions.length;i++){
    play(levelTwoQuestions[i].question,levelTwoQuestions[i].answer,levelTwoQuestions[i].correctAnswer);
  }
  totalScore(score);
}
...
if(levelTwoScore >= 41){
  ...
  for(var i=0;i<levelThreeQuestions.length;i++){
    play(levelThreeQuestions[i].question,levelThreeQuestions[i].answer,levelThreeQuestions[i].correctAnswer);
  }
  totalScore(score);
}
...
function totalScore(finalScore){
  for(var k=0;k<highScorers.length;k++){
    if(finalScore > 0 && finalScore >=23 && finalScore <=30 ){
      ...
      console.log(chalk.bold.bgRedBright("Kindly Share The Screenshot of Your Score."))
      break;
    }
